...
local function creatureSayCallback(npc, creature, type, message)
	...
	-- For VIP players, calculate price for all 7 blessings (5 regular + 2 enhanced)
	if isVip then
		local donthavefilter = function(p, b)
			return not p:hasBlessing(b)
		end
		local allFilter = function(b)
			return b.losscount
		end
		local missingAll = player:getBlessings(allFilter, donthavefilter)
		missing = #missingAll
		totalBlessPrice = 0
		for _, bless in ipairs(missingAll) do
			totalBlessPrice = totalBlessPrice + Blessings.getBlessingCost(player:getLevel(), false, bless.id >= 7) * Blessings.Config.InquisitonBlessPriceMultiplier
		end
	end

	local totalBlessCount = isVip and 7 or 5
	...
	elseif MsgContains(message, "blessing") or MsgContains(message, "bless") then
		if player:getStorageValue(Storage.Quest.U8_2.TheInquisitionQuest.Questline) == 25 then --if quest is done
			npcHandler:say("Do you want to receive the blessing of the inquisition - which means " .. (missing == totalBlessCount and "all " .. totalBlessCount .. " available" or missing) .. " blessings - for " .. totalBlessPrice .. " gold?", npc, creature)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 7 then
			...
			elseif player:removeMoneyBank(totalBlessPrice) then
				if isVip then
					npcHandler:say("You have been blessed with all seven blessings!, |PLAYERNAME|.", npc, creature)
					-- Add all 7 blessings (regular ids 2-6 + enhanced ids 7-8), but NOT Twist of Fate (id 1)
					for k, v in pairs(Blessings.All) do
						if (v.type == Blessings.Types.REGULAR or v.type == Blessings.Types.ENHANCED) and not player:hasBlessing(k) then
							player:addBlessing(k, 1)
						end
					end
					player:sendBlessStatus()
				else
					npcHandler:say("You have been blessed by all of five gods!, |PLAYERNAME|.", npc, creature)
					player:addMissingBless(false) -- only 5 regular blessings
				end
