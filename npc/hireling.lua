function createHirelingType(HirelingName)
	...
	local function getHirelingServiceString(creature)
		...
		-- Add trader-specific categories if hireling has TRADER skill
		if hireling:hasSkill(HIRELING_SKILLS.TRADER[2]) then
			str = str .. " I can offer you {potions} and {equipment}."
		end
	...
	local function creatureSayCallback(npc, creature, type, message)
		...
		-- Auto-focus: detectar nombres de sección en cualquier momento (sin importar el topic actual)
		...
		elseif MsgContains(message, "goods") then
			...
			return true
		elseif MsgContains(message, "potions") or MsgContains(message, "equipment")
			or MsgContains(message, "runes") or MsgContains(message, "distance")
			or MsgContains(message, "wands") or MsgContains(message, "rods")
			or MsgContains(message, "supplies") or MsgContains(message, "tools")
			or MsgContains(message, "postal") or MsgContains(message, "various")
			or MsgContains(message, "exercise weapons") then
			enableBankSystem[playerId] = nil
			local category = message:lower()
			-- Non-trader hirelings can only access "various"
			if not hireling:hasSkill(HIRELING_SKILLS.TRADER[2]) then
				if category ~= "various" then
					npcHandler:say("While I'm not a trader, I still have a collection of {various} items to sell if you like!", npc, creature)
					npcHandler:setTopic(playerId, TOPIC.GOODS)
					return true
				end
			end
			npcHandler:setTopic(playerId, TOPIC.GOODS)
			local categoryTable = itemsTable[category]
			if categoryTable then
				npcHandler:say("Here are the items for the category " .. message .. ".", npc, creature)
				npc:openShopWindowTable(player, categoryTable)
			end
			return true
		elseif MsgContains(message, "balance") or MsgContains(message, "deposit") or MsgContains(message, "withdraw") then
			local bankerSkillName = HIRELING_SKILLS.BANKER[2]
			if hireling:hasSkill(bankerSkillName) then
				npcHandler:setTopic(playerId, TOPIC.BANK)
				enableBankSystem[playerId] = true
				-- Don't return: fall through to bank parsing at the end of this function
			else
				enableBankSystem[playerId] = nil
				sendSkillNotLearned(npc, creature, bankerSkillName)
				return true
	...
	-- Custom greeting callback: include hireling skills in the greeting itself
	local function greetCallback(npc, creature)
		if not hireling then return true end
		...
		-- Build greeting with abilities included
		local servicesMsg = getHirelingServiceString(creature)
		local greeting = "It is good to see you. I'm always at your {service}. " .. servicesMsg
		npcHandler:setMessage(MESSAGE_GREET, greeting)
		npcHandler:setTopic(player:getId(), TOPIC.SERVICES)
